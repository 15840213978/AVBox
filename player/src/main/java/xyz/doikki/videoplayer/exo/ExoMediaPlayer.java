...
public class ExoMediaPlayer extends AbstractPlayer implements Player.Listener {
    ...
    /** 与 DefaultPreloadManager 共享的播放线程(可空,见 setPlaybackLooper) */
    ...
    public void initPlayer() {
        ...
        // media3 的 Builder 参数顺序为 renderersFactory, mediaSourceFactory, trackSelector(与 ExoPlayer2 相反)
        ...
        if (mPlaybackLooper != null) {
            builder.setPlaybackLooper(mPlaybackLooper);
        }
    ...
    /**
     * 预载对齐(2026-09-12):与 DefaultPreloadManager 的 preloadLooper 共享同一播放线程。
     * PreloadMediaSource.prepareSourceInternal() 对调用线程有硬校验(必须 == preloadHandler 的 looper),
     * 不注入则交接预载源时抛 IllegalStateException(播放立即失败→自动重试切内核)。
     * 必须在 initPlayer() 之前调用才生效(null = 不注入,保持 media3 默认 looper)。
     */
    public void setPlaybackLooper(Looper looper) {
        mPlaybackLooper = looper;
    }
