...
- **本地文件选择 = 系统 SAF**(2026-09-11 五轮定稿;**2026-09-16 改"直引原文件优先"**):`ActivityResultContracts.OpenDocument()`(MIME `*/*`)→ Uri 转 `clan://` 接口地址**回填链接输入框**(不直接保存)。打开选择器前先确保存储权限(Android 11+ = 系统「所有文件访问」,跳设置页;**未授权则每次点都跳**;用户拒绝/直接返回 ⇒ Toast「未开启『所有文件访问』,已取消导入(可再点一次重试)」并**取消本次导入**、不再弹选择器 —— 复制分支下同目录引用会静默失效,不替用户做这个决定)。转换规则 `LocalConfigHelper.localConfigToApi`:**直引条件缺一即复制** —— 有权限 + 能解析出真实路径(externalstorage 的 `primary:`/SD 卡、downloads 的 `raw:`/`msf:`(三段回退:Downloads→Files→`Download/<显示名>`)、media 的 `document:` 等,对齐 FongMi)+ 路径存在可读 + 落在 `/sdcard` 下(`RemoteServer` 的 `/file/` 只服务该根);直引才能让配置里 `./x.jar`、`../lib/x.js` 这类**同目录引用**解析到原目录。否则复制到外置私有 `files/config/<md5(uri)>_<原名>`(SAF 自带读取授权,**复制不需要存储权限**;md5 前缀防不同来源的同名配置互相覆盖)。⚠️ 落点是 files **不是 cache**:系统「清除缓存」会把副本删掉,之后只能静默回落到 `filesDir` 旧快照(用户改本地 json 不生效且无提示)。复制件旁没有兄弟文件,同目录引用会 404。**本地源无权限时拉取失败不再回落旧快照**(`ApiConfig.isLocalSourceUnreadable`,只判 `clan://localhost/`/`file://` 本机形态,`clan://<ip>/` 局域网源不在此列):直接报「本地源文件读不到 / 请开启『所有文件访问』后重试(或重新导入本地源)」—— 避免"改了 json 不生效却毫无提示"。自绘 `LocalFileActivity` 已整页删除。
